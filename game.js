(function () {
    ...
    function replay(){
        var shouldHide = document.getElementById('shouldHide');
        shouldHide.style.display = 'block';
    }
    function build() {
        console.log('123');
...
//                    alert.style.display = 'block';
                $.ajax({  
                    ...
                    url : "http://106.15.228.215:8080/game/game/"+score+"/updating",  
                    dataType : "json",
                    success : function(data){
                        var incontroll = 0;
                         $.ajax({  
                            ...
                            url : "http://106.15.228.215:8080/game/game/location",  
                            dataType : "json",
                            success : function(data){
                                incontroll = data.id;
                                $("#successIn>.score").html(score);
                        if(incontroll == 1){
                           $("#successIn>.successin").html("进入排行榜成功") 
                        }else{
                            $("#successIn>.successin").html("未进入排行榜") 
                        }
                        $("#successIn").css({"display":"block"});
                                 $.ajax({  
                            type : "get",
                            xhrFields:{ withCredentials:true},
                            crossDomain: true,
                         ...
                         $.ajax({  
                            ...
                            dataType : "json",
                            success : function(data){  
                                $("#userinfo>.maxcount").html(data.data);
                            },  
                            error:function(){  
                                alert('fail');  
                            }  
